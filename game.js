...
const DataModelValidator = {
  ...
  validateGameSettings(settings) {
    ...
    if (
      !Number.isInteger(settings.envelopeCount) ||
      settings.envelopeCount <= 0
    ) {
  ...
  validateGameState(gameState) {
    ...
    if (gameState.settings !== null) {
      ...
      if (!settingsValidation.isValid) {
        errors.push(
          ...settingsValidation.errors.map((err) => `settings.${err}`),
        );
      }
    }
  ...
  validateAudioAssets(audioAssets) {
    ...
    requiredSounds.forEach((soundKey) => {
      if (
        typeof audioAssets[soundKey] !== "string" ||
        audioAssets[soundKey].length === 0
      ) {
    ...
    if (
      audioAssets.backgroundMusic !== undefined &&
      (typeof audioAssets.backgroundMusic !== "string" ||
        audioAssets.backgroundMusic.length === 0)
    ) {
...
const UIManager = {
  ...
  validateSettingsForm() {
    const envelopeCount = parseInt(
      document.getElementById("envelope-count").value,
    );
    ...
    if (!hasErrors && minPrize > maxPrize) {
      fieldErrors.prizeRange =
        "เงินรางวัลต่ำสุดต้องน้อยกว่าหรือเท่ากับเงินรางวัลสูงสุด";
      hasErrors = true;
    }
  ...
  updateGameStats() {
    ...
    if (envelopeStatsElement) {
      envelopeStatsElement.innerHTML = `📦 เปิดแล้ว: ${stats.openedCount} / ${stats.totalEnvelopes}`;
    }
    ...
    try {
      ...
      if (stats.totalEnvelopes > 0) {
        const percentage = Math.round(
          (stats.openedCount / stats.totalEnvelopes) * 100,
        );
      ...
      console.warn(
        "Progress bar update failed (likely in test environment):",
        error.message,
      );
    }
  },
  ...
  showGameSummary(totalPrize) {
    ...
    const finalEnvelopeCountElement = document.getElementById(
    ...
    if (completionElement) {
      completionElement.textContent = stats.isGameComplete
        ? "เกมเสร็จสมบูรณ์!"
        : "เกมยังไม่เสร็จ";
    }
  ...
  animateStatCards() {
    try {
      ...
      // Silently handle DOM errors in test environment
      console.warn(
        "Stat card animation failed (likely in test environment):",
        error.message,
      );
    }
  },
...
const AudioManager = {
  ...
  async preloadAudioFiles() {
    try {
      ...
      if (!this.audioAssets.openEnvelopeSound) {
        ...
        this.audioAssets.openEnvelopeSound =
          await this.createEnvelopeOpenSound();
      }
      if (!this.audioAssets.specialPrizeSound) {
        this.audioAssets.specialPrizeSound =
          await this.createSpecialPrizeSound();
      }
  ...
  playSpecialPrizeSound(prizeAmount) {
    ...
    if (GameState.settings) {
      const prizeRange =
        GameState.settings.maxPrize - GameState.settings.minPrize;
  ...
  async initialize() {
    try {
      ...
      this.showAudioLoadingStatus(
        "ไม่สามารถโหลดเสียงได้ (ใช้เสียงสำรอง)",
        "warning",
      );
  ...
  getAudioStatus() {
    return {
      ...
      contextState: this.audioContext
        ...
        : "not-initialized",
      hasRealAudioFiles: Object.values(this.audioAssets).some(
        (asset) => asset && asset.constructor.name === "AudioBuffer",
      ),
    };
  },
};
  ...
  cleanupSparkleEffect(targetElement = null) {
    if (targetElement) {
      const sparkleContainer =
        targetElement.querySelector(".sparkle-container");
      if (sparkleContainer) {
...
const GameController = {
  ...
  initializeGame(settings) {
    ...
    GameState.envelopes = EnvelopeGenerator.generateEnvelopes(
      settings.envelopeCount,
    );
    ...
    // Enable new game button at the start of the game
    const newGameBtn = document.getElementById("new-game-btn");
    if (newGameBtn) {
      newGameBtn.disabled = false;
      newGameBtn.title = "เริ่มเกมใหม่";
    }
  },
  ...
  async openEnvelope(envelopeId) {
    ...
    // Play special effect animation for high prizes
    if (playedSpecialSound) {
      await AnimationManager.playSpecialEffectAnimation(
        prizeAmount,
        envelopeId,
      );
    }
    ...
    // Update new game button state based on prize amount
    this.updateNewGameButtonState();
  ...
  getGameStatistics() {
    // Handle corrupted or null envelopes gracefully
    const envelopes = GameState.envelopes || [];
    const openedEnvelopes = envelopes.filter(
      (envelope) => envelope && envelope.isOpened,
    );
    const remainingEnvelopes = envelopes.filter(
      (envelope) => envelope && !envelope.isOpened,
    );
    ...
    return {
      ...
      averagePrize:
        openedEnvelopes.length > 0
          ? (GameState.totalPrize || 0) / openedEnvelopes.length
          : 0,
      isGameComplete:
        envelopes.length > 0 && openedEnvelopes.length === envelopes.length,
      gamePhase: GameState.phase || "settings",
    };
  },

  /**
   * Update new game button state based on current prize amount
   * Rule: Enable if prize < 200, Disable if prize >= 200
   */
  updateNewGameButtonState() {
    const newGameBtn = document.getElementById("new-game-btn");
    if (newGameBtn) {
      // If total prize is less than 200, enable the button; otherwise disable it
      if (GameState.totalPrize < GameState.settings.minPrize + 50) {
        newGameBtn.disabled = false;
        newGameBtn.title = "เริ่มเกมใหม่";
      } else {
        newGameBtn.disabled = true;
        newGameBtn.title =
          "ไม่สามารถเริ่มเกมใหม่ได้ (ได้รับเงิน >= " +
          (GameState.settings.minPrize + 50) +
          " บาท)";
      }
    }
  },
    ...
    return {
      ...
      completionPercentage:
        stats.totalEnvelopes > 0
          ? Math.round((stats.openedCount / stats.totalEnvelopes) * 100)
          : 0,
...
const IntegrationManager = {
  ...
  validateGameStateConsistency() {
    try {
      ...
      const actualOpenedCount = GameState.envelopes.filter(
        (env) => env.isOpened,
      ).length;
      ...
      GameState.envelopes.forEach((envelope) => {
        ...
        if (!validation.isValid) {
          console.warn(
            `Envelope ${envelope.id} validation failed:`,
            validation.errors,
          );
        }
      });
  ...
  updateResponsiveElements() {
    ...
    if (envelopesContainer) {
      if (isMobile) {
        ...
        envelopesContainer.style.gridTemplateColumns =
    ...
    try {
      ...
      if (GameState.envelopes.length > 100) {
        healthReport.warnings.push(
          "Large number of envelopes may impact performance",
        );
      }
      ...
      requiredElements.forEach((elementId) => {
        if (!document.getElementById(elementId)) {
          healthReport.issues.push(
            `Missing required DOM element: ${elementId}`,
          );
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  settingsInputs.forEach((inputId) => {
    ...
    if (input) {
      ...
      input.addEventListener("input", function () {
        ...
        const allInputs = settingsInputs.map((id) =>
        ...
        if (!formValidation.isValid) {
          ...
          Object.keys(formValidation.fieldErrors).forEach((fieldName) => {
            ...
            switch (fieldName) {
              ...
              case "prizeRange":
                shouldShow =
                  document.getElementById("min-prize").value.trim() &&
                  document.getElementById("max-prize").value.trim();
                break;
            }

            if (shouldShow) {
              currentFieldErrors[fieldName] =
                formValidation.fieldErrors[fieldName];
            }
          });
  ...
  window.addEventListener("beforeunload", function (event) {
    if (GameState.phase === "playing" && GameState.openedCount > 0) {
      const message =
        "คุณมีเกมที่กำลังเล่นอยู่ คุณแน่ใจหรือไม่ที่จะออกจากหน้านี้?";
  ...
  console.log(
    "Keyboard shortcuts: Ctrl+S (sound), Ctrl+N (new game), Ctrl+R (reset), Esc (back), F1 (help)",
  );
});
